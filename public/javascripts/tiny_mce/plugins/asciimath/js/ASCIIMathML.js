...
function simpleLaTeXformatting(st) {
  ...
  //st = st.replace(/textit/g, "emph");
  st = replace_regex(st, /\\emph{.*?}/, 5, "<em>", "</em>");
  //st = st.replace(/\\textbf{(.*?)}/g,"<b>$1</b>");
  st = replace_regex(st, /\\textbf{.*?}/, 7, "<b>", "</b>");
  //st = st.replace(/\\cite{(.*?)}/g,"[$1]");
  st = replace_regex(st, /\\cite{.*?}/, 5, "", "");
  //st = st.replace(/\\chapter{(.*?)}/g,"<h2>$1</h2>");
  st = replace_regex(st, /\\chapter{.*?}/, 8, "<h2>", "</h2>");
  //st = st.replace(/\\subsection{(.*?)}/g,"<h4>$1</h4>");  
  st = replace_regex(st, /\\subsection{.*?}/, 11, "<h4>", "</h4>");
...
function LMprocessNode(n, linebreaks, spanclassLM) {
  ...
  if (spanclassLM!=null) {
    ...
    //Because if elements like \emph are present then also this function should be called. and $ is not a requirement in that case.
    if ((st!=null || st.indexOf("\$ ")!=-1 || st.indexOf("\$<")!=-1 ||
...
var defaultstroke = "black";   // default line color
...
// function mathMLDisplay() {
//   var elements = $$(".convertMath");
//   var element;
//   if (elements.length > 0) {
//     //if (translateLaTeX) LMtranslate(); // Translates full body.
//     //if (translateASCIIMath) translate();
//     //if (translateASCIIsvg) drawPictures();
//     for (var ii=0; ii < elements.length; ii++) {
//       element = elements[ii];
//       LMprocessNode(element, false, null);
//     }
//   }
// }
var LMsymbolsInitialized = false;
...
function replace_regex(st, regex_to_search, length, start_tag, end_tag)
{
  index = st.search(regex_to_search);
  start = [];
  if(index < 0)
  {
    return st;
  ...
  {
    for(var i=index+length; i<st.length; i++)
    {
      if(st[i]=='{')
        start.push('{')
      if(st[i] == '}')
    ...
    match = st.substring(index+length+1, i);
    match_to_replace = st.substring(index, i+1);
    st = st.replace(match_to_replace, start_tag + match + end_tag);
    st = replace_regex(st, regex_to_search, length, start_tag, end_tag);
